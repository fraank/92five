...
var EventsView = Backbone.View.extend({
    template: _.template('<div class=row-fluid><div class="span5 time_listing_1" >' + '<% if (allday == true) { %> allday <% } else { %> <%=start_time%>' + ' - ' + '<%=end_time%> <% } %>' + '</div><div class="span7 time_listing_1" ><a data-toggle=modal class=cal_event_title eventid=' + '<%=id%>' + ' href=#myModal4>' + '<%=title%>' + '</a></div></div>' + '<div class="calender-viewevent hide">' + '<% if (editdelete != "no") { %>' + '<div class="p-icon-inner"><a class="p-icon-1" title="Edit Event" href=calendar/event/edit/' + '<%=id%>' + '><img alt="" src=' + tempurl + 'assets/images/dashboard/p-edit.png></a><a class="p-icon-1" title="Delete Event" href=' + '#' + '><img alt="" class="delevent" eventid=' + '<%=id%>' + ' src=' + tempurl + 'assets/images/dashboard/p-delete.png></a></div>' + '<% } %>' + '<div class="viewevent-detail-inner"><div class="viewevent-left"><div class="viewevent-detail-1">Category:<span class="viewevent-note">' + '<%=category%>' + '</span></div><div class="viewevent-detail-1">Note: <span class="viewevent-note">' + '<%=notes%>' + '</span></div><div class="viewevent-detail-1">Location: <span class="viewevent-note">' + '<%=location%>' + '</span></div></div><div class="viewevent-right"><div class="viewevent-asignee"><label>People:</label><div class="viewevent-asignee-right">' + '<% _.each(users, function(user) { %>' + '<div class="viewevent-detail-3">' + '<%=user.first_name%> <%=user.last_name%>' + '</div>' + '<% }); %>' + '</div></div></div></div></div>'),
    initialize: function() {
